  ...
  SMSUITE_DATA.forEach(plugin => {
    ...
    if (plugin.content) {
      card.addEventListener("click", () => {
        ...
        if (plugin.icon) {
          const imgHtml = `<div style="float: right; margin-left: 2rem; margin-top: -0.5rem;"><img src="${window.ASSET_PREFIX || ''}${plugin.icon}" alt="icon" style="width: 200px; height: auto;" /></div>`;
    ...
    else if (plugin.group === "communicators") communicatorsGrid.appendChild(card);
